...
@Obfuscate
class UpdateCheckOutAddressFragmentVM : BaseViewModel<UpdateCheckOutAddressFragmentNavigator>() {
...
*/
    ...
    fun updateDeliverAddressApiCall(
        ...
        Log.e("CheckOutAddressFragmentVM:", "txvAddress2:- $strAddressType")
        if (!navigator!!.isConnectedToInternet()) {
            return
        }
        viewModelScope.launch {
            navigator?.showProgress()
            val result = Repository.get.updatdeNewDeliverAddress(
                AppConstants.CHANNEL_MODE, updategetRequestParams(
                    Id,
                    strFullName,
                    strEmail,
                    strMobileNumber,
            ...
            result.fold(
                ...
                },
                {
                    if (it.status.equals("success")) {
                        getAllDeliverAddressDataResponse.value = it
                        navigator?.updateDeliverAddressApiCall()
                    }
                }
            )
        }
    ...
    private fun updategetRequestParams(
        addressId: Int,
        strFullName: String,
        strEmail: String,
        strMobileNumber: String,
